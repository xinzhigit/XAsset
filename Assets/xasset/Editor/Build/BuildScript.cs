    ...
    public static class BuildScript
    {
        ...
        public static void CopyToStreamingAssets(params BuildRecord[] records)
        {
            ...
            {  
                ...
                {
                    CreateBinary(bundles, destinationDir);
            ...
            versions.offlineMode = settings.scriptPlayMode == ScriptPlayMode.Preload;
            File.WriteAllText($"{destinationDir}/{Versions.Filename}", JsonUtility.ToJson(versions));
        }

        public static void CreateBinary(List<ManifestBundle> bundles, string dir, List<string> rawBundles = null)
        {
            var savePath = Path.Combine(dir, Versions.EncryptionData);
                ...
                using (var writer = new BinaryWriter(stream))
                {
                    foreach (var bundle in bundles)
                    {
                        ...
                        else
                        {
                            if (rawBundles == null)
                            {
                                Copy(bundle.nameWithAppendHash, dir); 
                            }
                            else
                            {
                                rawBundles.Add(bundle.nameWithAppendHash);
                            }
                        }
                    }
                }
