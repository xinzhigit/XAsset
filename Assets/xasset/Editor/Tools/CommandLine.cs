    ...
    public static class CommandLine
    {
        ...
        public static void BuildPlayer()
        {
            ...
            if (!string.IsNullOrEmpty(config))
            {
                ...
                {
                    if (!splitConfig.name.Equals(config)) continue;
                    settings.splitConfig = splitConfig;
                    break;
                }
            }
            
            var offlineModeEnabled = !string.IsNullOrEmpty(offlineMode) && bool.TryParse(offlineMode, out var value) && value;
            settings.scriptPlayMode = offlineModeEnabled ? ScriptPlayMode.Preload : ScriptPlayMode.Increment;
            settings.Save();
